...
Adding more consumers running in parallel will not increase the throughput.

The general rule is splitting a \kftopic across more \kfpartitions in the \kfbroker means it can be spread across more \kfconsumers downstream, and the number of \kfpartitions a \kftopic is assigned in the \kfbroker sets the upper limit on the number of concurrent \kfconsumers downstream.
...
When a group of \kfconsumers subscribe to a \kftopic, the \kfbroker assigns the available \kfpartitions to \kfconsumers and sets up an offset pointer in each \kfpartition to keep track of which messages have been sent to that \kfconsumer group.
...
When the \kfbroker receives an acknowledgment from the \kfconsumer, the \kfbroker marks those messages as received by moving the offset pointer to point to the next block of messages.
...
If we have 122 \kfpartitions and 122 consumer threads, then in theory each \kfconsumer should only need to handle data for one \kfpartition, or 1/122 of the data.
