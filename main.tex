...
The data generation and \crossmatch tests are centred on a single target position for a test run, configured using \texttt{target.ra} and \texttt{target.dec} properties which set the right ascension (\textit{ra}) and declination (\textit{dec}) of the target position in degrees.
...
[[diagram]]
\begin{itemize}
    \item A \inlinecode{CrossmatchComponent} that generates a \kfstream of \crossmatch annotated candidates :
    \begin{itemize}
        \item \inlinecode{CatalogMatcher} matches against a catalog and adds annotations.
    \end{itemize}
    \begin{itemize}
        \item \inlinecode{KafkaProducer} writes annotated candidates to a new \kftopic.
    \end{itemize}
\end{itemize}
...
This simple \yaml configuration would deploy a new \kafka \kfbroker service, consisting of 4 server instances along with the associated \zookeeper service and any monitoring and management services needed to administer them.
...
\kafka was designed as a reliable publish/subscribe message distribution service for sequential streams of data.

When describing \kafka systems we often refer to three components, a \kfproducer which produces messages, a \kfconsumer that consumes messages and a \kfbroker that acts as the store and forward delivery system, accepting messages from producers and delivering messages to consumers.

\kafka itself is the part in the middle, the broker that provides the message delivery system.
The \kfproducer and \kfconsumer are clients, external and separate from the \kafka service.

The \kafka \kfbroker does not know where the messages come from, or what happens to them after they are delivered.
The \kafka \kfbroker concentrates purely on listening for messages from the producers, storing them in a buffer and then forwarding them on to consumer.
...
This separation between the delivery system and the message producing and consuming clients is described as the 
publish-subscribe pattern \footurl{https://en.wikipedia.org/wiki/Publish-subscribe_pattern}.

In terms of system architecture, the publishing component of the publish-subscribe pattern is referred to as a \kfpublisher, or in \kafka terms, a \kfproducer.

   {Pub} -> [broker]

In the publish-subscribe pattern, a \kfpublisher does not send messages to a specific destination, instead messages are tagged as belonging to a topic and sent to a delivery service or \kfbroker. Clients who wish to receive messages contact the broker and subscribe to a topic. The broker will then pass on messages it receives that have been tagged as belonging to that topic.

The choice of which messages belong to which topics is made by the \kfpublisher when they assign specific messages to specific topics. The broker does not read, interpret or filter the messages. It just passes messages on to the \kfsubscribers based on topic the messages have been tagged with. 
...
\confluent have defined a REST \webservice interface for a schema registration service \footurl{https://www.confluent.io/confluent-schema-registry/} that allows \kfproducers to upload and register their message schema and get a numeric identifier.
If a \kfproducer uses a schema registry to generate the identifier sent in the message header, and the client \kfconsumer is configured to use the same registry service, then the \kfconsumer can use the registry to resolve the identifier and download a copy of the schema.
The \kfconsumer can use this downloaded schema to create a generic \deserzer that can cope with the new message structure.
This \deserzer will not be as efficient as the compiled \deserzers used for known schema, but it gives the client the option of being able to handle new message types without having to ship a full copy of the schema with each message.
...
The schema registration service is designed to manage all the schema for an entire organisation, including support for deploying a distributed registry service that functions across more than one data center.

However, on its own it does not work as well for managing schema across multiple organisations.
...
The scheme relies on the \kfproducers and \kfconsumers within an organisation all being configured with the endpoint address of the \schemaregistry they should use to register and resolve schema and schema identifiers.

Putting these two issues together means that within an organisation \kfproducers and \kfconsumers can use this system to exchange messages with flexible message schema. However, the scheme does not scale to be able to register and resolve schema and schema identifiers across separate organisations.
...
When a \kfproducer starts to send messages it first checks the registry to see if the schema it is going to use has been registered. If the schema has not been registered yet the \kfproducer uploads a \json encoded copy of the schema and in return receives a new unique identifier for the schema, e.g. 27.

The first time a \kfconsumer receives a message with schema identifier 27 in the header it can use the \schemaregistry to resolve this identifier into a \json encoded copy of the schema and use that to generate a \deserzer to handle the new  message structure.
...
However if the \kfproducer and \kfconsumer are in different organisations, then when the \kfconsumer receives a message with schema identifier 27 in the header it has no way of discovering which \schemaregistry it should use to resolve the identifier and get a copy of the original schema.
...
The secondary or slave registries pass new schema back to the primary registry for registration, centralising the  generation of identifiers and ensuring they remain unique within the organisation.

The requirement for the secondary or slave registries pass new schema back to the primary registry for registration means that while this model may work for multiple (\textasciitilde 10) data centers within an organization, it is unlikely to scale to include a multi-level hierarchy of (\textasciitilde 100) different organizations within the astronomy domain.
...
This could be handled by deploying a gateway \kfbroker, a modified form of the mirror maker service that translates schema identifiers on the fly as it imported messages from the upstream \lsst \kfbroker into the \lsstuk system.

If the \lsst project do decide to use the \confluent \schemaregistry to manage schema versions, then their messages will be transmitted on the wire using the \confluent wire format.\footurl{https://docs.confluent.io/current/schema-registry/serdes-develop/index.html#wire-format}

This consists of a single byte to represent the serialization format version (zero in the current version), followed by four bytes for the schema identifier and then the rest of the message body.
...
The \lsstuk gateway \kfbroker would maintain a mapping between schema identifiers from the upstream \lsst \kfbroker and the internal \lsstuk \schemaregistry.
The gateway \kfbroker would read the four byte schema identifier from each message and check for a corresponding schema in the local \schemaregistry. If it did not find a matching schema then it would call the upstream \schemaregistry to resolve the schema identifier, download a \json encoded copy of the schema and then register the new schema in the local \schemaregistry. This would generate a new identifier, valid for the local domain.
...
Using a gateway \kfbroker like this enables the \lsstuk project to maintain an internal \schemaregistry which provides version control for all of the message schema used within the \lsstuk system. This would include copies of schema published by the upstream \kfbroker and the internal schema generated by dynamic database queries and filtering components within the \lsstuk system.
